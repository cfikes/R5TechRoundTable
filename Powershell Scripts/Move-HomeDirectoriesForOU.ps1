...
A///AAIACw=='))
    ...
    return $SelectedObject | select -ExpandProperty DistinguishedName

} #End Function Choose-ADOrganizationalUnit

function Get-InputTextbox($WinTitle, $WinMessage) {
	...
	if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
		$x = $textBox.Text
		if ($x -eq "") {
			[System.Windows.Forms.MessageBox]::Show("You didn't enter anything!", "Error")
			exit
		}
		else {
			return $x
...
Function Get-Folder($initialDirectory) {
    ...
    $foldername.Description = "Choose the new Root for Home Directories"
...
$DestFolder = Get-Folder


$UserList = Get-ADUser -Filter 'enabled -eq $true' -Properties ProfilePath,HomeDirectory,HomeDrive -SearchBase "$OU" | 
Select Name, SamAccountName, ProfilePath, HomeDirectory, HomeDrive | 
Sort-Object Name | 

ForEach-Object {
    if(!$_.HomeDirectory){
        Write-Host "No Home Directory for " $_.SamAccountName
    } else {
        <# Correct Permissions and Copy Contents #>

        $SourceDIR = $_.HomeDirectory
        $HomeUser = $_.SamAccountName
        $SourceFolder = Get-Item $SourceDIR
        $DestDir = $DestFolder
        $DestACLPath = "$DestDir\$HomeUser"
...
"@  #It has to be at CHAR 1
        
        add-type $AdjustTokenPrivileges
        
        #Seize Permissions to Make Changes
        ...
        $SourceFolder.SetAccessControl($NewOwnerACL)

        #Fix Permissions to Allow Copy and Set Owner as Home dir User
        ...
        $AR = New-Object  system.security.accesscontrol.filesystemaccessrule($HomeUser,"FullControl","Allow")
        $ACL.AddAccessRule($AR)
        Set-Acl $SourceDIR $ACL

        #Copy Contents Over
        Copy-Item -Path $SourceDIR -Destination $DestDir -Recurse -Verbose


        $ACL = Get-Acl $DestACLPath
        $ACL.SetOwner([System.Security.Principal.NTAccount]"BUILTIN\Administrators")
        Set-ACL $DestACLPath $ACL
        ...
        #Set Permissions on New Folder
        #$emptyACL = New-Object System.Security.AccessControl.DirectorySecurity
        $ACL = Get-Acl $DestACLPath
        $AR = New-Object  system.security.accesscontrol.filesystemaccessrule("Domain Admins","FullControl","ContainerInherit,ObjectInherit","NoPropagateInherit","Allow")
        $ACL.AddAccessRule($AR)
        $AR = New-Object  system.security.accesscontrol.filesystemaccessrule("$HomeUser","FullControl","ContainerInherit,ObjectInherit","NoPropagateInherit","Allow")
        $ACL.AddAccessRule($AR)
        Set-Acl $DestACLPath $ACL
    
        Write-Host $_.SamAccountName  $_.HomeDirectory
    }
