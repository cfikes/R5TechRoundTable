function Choose-ADOrganizationalUnit
{
	...
	function Get-NextLevel
	{
		...
		else
		{
            ...
            $ADsearcher.filter = "(|(objectClass=organizationalUnit)(objectClass=Computer)(ObjectClass=container)(ObjectClass=builtinDomain))"
...
A///AAIACw=='))
    ...
    return $SelectedObject | select -ExpandProperty Name

} #End Function Choose-ADOrganizationalUnit

$Computer = Choose-ADOrganizationalUnit
$SSRM = "C:\Program Files (x86)\Microsoft Configuration Manager\AdminConsole\bin\i386\CmRcViewer.exe"
Start-Process -FilePath $SSRM -ArgumentList $Computer