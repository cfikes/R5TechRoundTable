<#
	Executes the one line command against all computers in selected OU.
#>
...
function Choose-AzureADList($listObject) {
    ...
    foreach ($GROUP in $listObject){
        [void] $listBox.Items.Add($GROUP.DisplayName)
    }
...
#Window Title
$Host.UI.RawUI.WindowTitle = "Processing Commands against " + $OU

"Executing against $OU"
...
#Select Group
$WorkingList = Choose-AzureAdList $AllGroups

$WorkingListID = Get-AzureADGroup -SearchString $WorkingList | Select-Object ObjectId -ExpandProperty ObjectId


foreach ($USER in $(Get-ADUser -Filter 'enabled -eq "true"' -SearchBase "$OU" -properties * | Select Name,mail)){
	if($USER.mail) {
		$ThisUser = Get-AzureADUser -SearchString $USER.mail
		"Adding " + $USER.mail
		Add-AzureADGroupMember -ObjectId $WorkingListID.ObjectId -RefObjectId $ThisUser.ObjectId
     }
}
