...
foreach ($USER in $(Get-ADUser -Filter * -SearchBase "$OU" -properties * | Select Name,mail)){
	if($USER.mail) {
		$ThisUser = Get-AzureADUser -SearchString $USER.mail
		"Removing " + $USER.mail
		Remove-AzureADGroupMember -ObjectId $WorkingListID.ObjectId -MemberId $ThisUser.ObjectId
	}
}
